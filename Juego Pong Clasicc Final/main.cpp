...
main()
{
	...
	fscanf(archivo,"%d : %d", &marc_p1,&marc_p2);
	if((marc_p1 == 3) || (marc_p2 == 3)){
		marc_p1 = 0;
		marc_p2 = 0;
	}
	fclose(archivo);
...
void *marcador(void *args){
	while(1==1){
		...
		if(marc_p1 == 1 && marc_p2 == 0){
			outtextxy((XMin + 20),660, "1");
			outtextxy((XMax - 50),660, "0");
		}
		if(marc_p1 == 2 && marc_p2 == 0){
			outtextxy((XMin + 20),660, "2");
			outtextxy((XMax - 50),660, "0");
		}
		if(marc_p1 == 0 && marc_p2 == 1){
			outtextxy((XMin + 20),660, "0");
			outtextxy((XMax - 50),660, "1");
		}
		if(marc_p1 == 0 && marc_p2 == 2){
			outtextxy((XMin + 20),660, "0");
			outtextxy((XMax - 50),660, "2");
		}
		...
		if((marc_p1 == 3) || (marc_p2 == 3)){
			printf("Marcador %d : %d\n", marc_p1,marc_p2);
			if(marc_p1 == 3)
				printf("GANADOR PLAYER 1\n");
			else
				printf("GANADOR PLAYER 2\n");
			FILE *archivo;
			archivo = fopen("marcador.txt","wt");
			fprintf(archivo,"%d : %d",marc_p1,marc_p2);
			fclose(archivo);
			closegraph();
		}
		delay(300);
